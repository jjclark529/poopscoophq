...
const navSections: NavSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        label: "Ad Quick Launch",
        href: "/dashboard/ad-builder",
        icon: (
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function Sidebar() {
  ...
  return (
    <aside className={`${collapsed ? "w-16" : "w-64"} bg-white border-r border-gray-200 flex flex-col min-h-screen transition-all duration-200`}>
      {/* Logo */}
      <div className="p-4 border-b border-gray-100">
        <Link href="/dashboard" className="flex items-center gap-3">
          <Image src="/logo.png" alt="PoopScoop HQ" width={32} height={32} className="object-contain" />
          {!collapsed && (
            <h1 className="text-gray-900 font-bold text-lg leading-tight truncate">PoopScoop HQ</h1>
          )}
        </Link>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto p-3 space-y-5">
        {navSections.map((section) => (
          <div key={section.title}>
            {!collapsed && (
              <p className="text-gray-400 text-[10px] font-semibold uppercase tracking-wider mb-2 px-3">
                {section.title}
              </p>
            )}
      ...
      <div className="px-3 pb-2">
        <button
          ...
          className="w-full flex items-center justify-center gap-2 px-3 py-1.5 rounded-lg text-xs text-gray-400 hover:text-gray-600 hover:bg-gray-50 transition-colors"
        >
          {collapsed ? "→" : "← Collapse"}
        </button>
      </div>
      ...
      <div className="p-3 border-t border-gray-100">
        <div className="flex items-center gap-3 px-2 py-1.5">
          ...
          {!collapsed && (
            <div>
              <p className="text-gray-900 text-sm font-medium">Doctor Doo</p>
              <p className="text-gray-400 text-xs">Riverside, CA</p>
            </div>
          )}
        </div>
      </div>
